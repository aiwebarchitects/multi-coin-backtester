"""
RSI 5min Double Confirm Algorithm Implementation
Requires TWO consecutive oversold signals on 5-minute candles before buying
Combines the benefits of 5-minute timeframe with double confirmation
"""
...
class RSI5minDoubleConfirmAlgorithm(BaseAlgorithm):
    """RSI trading algorithm with double confirmation on 5-minute candles"""
    
    def __init__(self, period: int = 14, oversold_threshold: int = 30, 
                 overbought_threshold: int = 70, **kwargs):
        super().__init__("RSI_5MIN_DOUBLE_CONFIRM", **kwargs)
        self.period = period
    ...
    def get_parameter_ranges(self) -> Dict:
        """
        Get parameter ranges for RSI 5min Double Confirm optimization
        
        Optimized for 5-minute timeframe with double confirmation
        """
        return {
    ...
    def __str__(self):
        return f"RSI_5MIN_DOUBLE_CONFIRM(period={self.period}, oversold={self.oversold_threshold}, overbought={self.overbought_threshold})"
