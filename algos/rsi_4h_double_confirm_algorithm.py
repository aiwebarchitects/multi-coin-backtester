"""
RSI 4H Double Confirm Algorithm Implementation
Requires TWO consecutive oversold signals on 4-hour candles before buying
Combines the benefits of 4-hour timeframe with double confirmation
"""
...
class RSI4HDoubleConfirmAlgorithm(BaseAlgorithm):
    """RSI trading algorithm with double confirmation on 4-hour candles"""
    
    def __init__(self, period: int = 14, oversold_threshold: int = 30, 
                 overbought_threshold: int = 70, **kwargs):
        super().__init__("RSI_4H_DOUBLE_CONFIRM", **kwargs)
        self.period = period
    ...
    def get_parameter_ranges(self) -> Dict:
        """
        Get parameter ranges for RSI 4H Double Confirm optimization
        
        Optimized for 4-hour timeframe with double confirmation
        """
        return {
    ...
    def __str__(self):
        return f"RSI_4H_DOUBLE_CONFIRM(period={self.period}, oversold={self.oversold_threshold}, overbought={self.overbought_threshold})"
